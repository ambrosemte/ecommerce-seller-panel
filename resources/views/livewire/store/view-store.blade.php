            ...
            <div class="card shadow-sm">
                <div class="card-body d-flex align-items-center justify-content-between">
                    ...
                    <div class="d-flex align-items-center">
                        <!-- Store Logo -->
                        <img src="{{ $store['image_url'] ?? asset('default-logo.png') }}" alt="Store Logo"
                    ...
                    <!-- Buttons -->
                    <div>
                        <button wire:click="activateDeactivateStore('{{ $store['is_active'] }}')" class="btn btn-secondary btn-sm mr-5"
                            wire:loading.attr="disabled">
                            <span wire:loading.remove
                                wire:target="activateDeactivateStore('{{ $store['is_active'] }}')">
                                {{ $store['is_active'] ? 'Deactivate' : 'Activate' }}
                            </span>
                            <span wire:loading wire:target="activateDeactivateStore('{{ $store['is_active'] }}')">
                                <span class="spinner-border spinner-border-sm me-1"></span> Processing...
                            </span>
                        </button>

                        <button wire:click="deleteStore"
                            wire:confirm.prompt="Are you sure you want to delete this store?\n\nType DELETE to confirm|DELETE"
                            class="btn btn-danger btn-sm" wire:loading.attr="disabled">
                            <span wire:loading.remove wire:target="deleteStore">Delete</span>
                            <span wire:loading wire:target="deleteStore">
                                <span class="spinner-border spinner-border-sm me-1"></span> Processing...
                            </span>
                        </button>
                    </div>
